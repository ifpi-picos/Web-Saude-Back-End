...
consultasRouter.get('/especialidade/', async (req: Request, res: Response) => {
	try {
		...
		const filtro =
			await ConsultasRepository.filtrarUnidadesDeSaudePelaEspecialidade(nome);
...
consultasRouter.get(
	'/total-unidades-de-saude',
	async (req: Request, res: Response) => {
		try {
			const totalUnidadesDeSaude =
				await ConsultasRepository.contarTotalUnidadesDeSaude();
