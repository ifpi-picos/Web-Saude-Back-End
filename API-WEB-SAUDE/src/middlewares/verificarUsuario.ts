...
public verificarFuncionario = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!usuario || usuario.tipo !== 'funcionario') {
        return res.status(403).json({ Message: 'Acesso negado. Somente funcionários podem cadastrar clínicas.' });
      }
