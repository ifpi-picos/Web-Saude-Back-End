...
export default function calcularStatus(horarioSemana: {
	open: string;
	close: string;
}): boolean {
	if (horarioSemana && horarioSemana.open && horarioSemana.close) {
		try {
			...
			console.log(
				...
				format(horarioAbertura, formato),
			);
			console.log(
				'Horário de fechamento da clínica:',
				format(horarioFechamento, formato),
			);
			...
			if (diaAtual >= 0 && diaAtual <= 6) {
				...
				return (
					tempoAtualEmMinutos >= tempoAberturaEmMinutos &&
					tempoAtualEmMinutos <= tempoFechamentoEmMinutos
				);
			}
		} catch (error) {
