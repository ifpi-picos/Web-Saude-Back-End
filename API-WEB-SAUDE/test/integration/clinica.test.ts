...
describe('Testes para a rota de clinicas', () => {
	...
	it('deve alterar uma clínica existente', async () => {
		...
		const response = await request(app)
			.put(`/admin/alterar-clinica/${id}`)
			.send(clinicaAtualizadaData);
	...
	it('deve deletar uma clínica específica', async () => {
		...
		const token =
			'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI2NTMxYzJiZmY1OTE2Y2JkMTRkZTliMGMiLCJpYXQiOjE2OTgzNzI0MjcsImV4cCI6MTY5ODM3NjAyN30.GHkKI7xp31uvc4J6T2ix5LrfDJSzU5LMfkxXc61z6G4';
		const response = await request(app)
